...
def run_runner(project: str, environment: dict[str, str], command: str) -> None:
    require(
        ...
        "python",
        "tests/p4_rate_limit_smoke.py",
        command,
    )


def main() -> int:
    project = f"bankcore-p4-rate-limit-{uuid.uuid4().hex[:8]}"
    with tempfile.TemporaryDirectory(prefix="bankcore-p4-rate-limit-") as temporary_root:
        ...
        try:
            ...
            run_runner(project, environment, "distributed")
            require(project, environment, "stop", "redis")
            run_runner(project, environment, "fallback")
            require(project, environment, "up", "-d", "--wait", "redis")
            run_runner(project, environment, "recovery")
            exit_code = 0
        except Exception:
