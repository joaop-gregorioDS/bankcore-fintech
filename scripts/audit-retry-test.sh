...
project="bankcore-audit-retry-test-$(cat /proc/sys/kernel/random/uuid | tr -d '-')"
...
if docker info >/dev/null 2>&1; then
  ...
  if [ "$exit_code" -eq 0 ]; then docker compose -p "$project" -f "$compose_file" run --rm --no-deps audit-tests python -m pytest -q tests/p3f_retry_dlq_tests.py; exit_code=$?; fi
else
