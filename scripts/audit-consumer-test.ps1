...
$projectName = "bankcore-audit-test-$([guid]::NewGuid().ToString('N'))"
...
function Invoke-Compose {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Audit test environment step failed with exit code $LASTEXITCODE."
    }
}
...
try {
    ...
    & docker compose -p $projectName -f $composeFile run --rm --no-deps audit-tests
