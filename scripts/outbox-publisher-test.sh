...
compose_file="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)/docker-compose.outbox-test.yml"
project="bankcore-outbox-test-$(cat /proc/sys/kernel/random/uuid | tr -d '-')"
...
if docker info >/dev/null 2>&1; then
  ...
  if [ "$exit_code" -eq 0 ]; then docker compose -p "$project" -f "$compose_file" run --rm --no-deps publisher-tests; exit_code=$?; fi
else
