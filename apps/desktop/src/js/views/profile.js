...
export async function renderProfile(container, user, account, onLogout, showToast) {
  ...
  const currentApi = (await getSecureItem('api_endpoint')) || CONFIG.apiBaseUrl;
        ...
        <div style="display:flex; flex-direction:column; gap:20px;">
          ...
          <div class="card">
            ...
            <div class="form-group">
              <label class="form-label">Servidor da API Conectado</label>
              <select id="apiEndpointSelect" class="form-input" style="font-size:12px;">
                <option value="${CONFIG.apiBaseUrl}" ${currentApi === CONFIG.apiBaseUrl ? 'selected' : ''}>
                  HTTPS Oficial (${CONFIG.apiBaseUrl})
                </option>
                <option value="${CONFIG.labApiUrl}" ${currentApi === CONFIG.labApiUrl ? 'selected' : ''}>
                  Laboratório IP (${CONFIG.labApiUrl})
                </option>
                <option value="${CONFIG.localApiUrl}" ${currentApi === CONFIG.localApiUrl ? 'selected' : ''}>
                  Desenvolvimento Local (${CONFIG.localApiUrl})
                </option>
              </select>
            </div>
          </div>
  ...
  const apiSelect = container.querySelector('#apiEndpointSelect');
  apiSelect.addEventListener('change', async (e) => {
    const newEndpoint = e.target.value;
    await setSecureItem('api_endpoint', newEndpoint);
    showToast(`Servidor configurado: ${newEndpoint}`, 'success');
  });
